...
-- Ejecutar con: npx wrangler d1 execute siso-db --file schema_v2_indexes.sql
-- Fecha: 2026-08-16

-- Índice por updated_at (cleanup/snapshots/ORDER BY)
CREATE INDEX IF NOT EXISTS idx_updated_at     ON siso_store(updated_at);

-- Índices compuestos para multi-tenant (requiere columna tenant agregada primero)
...
-- Índices audit_log (solo si la tabla ya fue creada)
