...
/**
 * AIConfigPanel — Panel de configuración IA (contenido puro, sin overlay propio)
 * El caller es responsable del overlay/modal wrapper.
 * Props: aiConfig, onSave(cfg), onClose()
 */
export const AIConfigPanel = ({ aiConfig, onSave, onClose }) => {
  const [cfg, setCfg] = useState(() => ({
    activeProvider: aiConfig?.activeProvider || 'gemini',
    keys: { gemini: '', groq: '', together: '', openrouter: '', ...(aiConfig?.keys || {}) },
  }));
  ...
  const PROVIDER_INFO = {
    gemini: {
      ...
      steps: [
        '1️⃣ Clic en "Obtener key →"',
        '2️⃣ Inicia sesión con tu cuenta Google',
  ...
  const testProvider = async (providerKey) => {
    ...
    try {
      ...
      else if (msg.includes('CORS')) hint = ' → CORS bloqueado, usa otro proveedor';
  ...
  const activeInfo = PROVIDER_INFO[cfg.activeProvider];
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-700 p-4 text-white flex-shrink-0">
        ...
        {activeInfo && (
          <div className="mt-2 inline-flex items-center gap-1.5 bg-white/20 rounded-lg px-2 py-1">
            ...
            <span className="text-[10px] text-indigo-100 font-black">{activeInfo.label}</span>
          </div>
        )}
      </div>
      ...
      <div className="overflow-y-auto flex-1 p-4 space-y-3">
        {/* Status banner */}
        {anyWorking ? (
          <div className="bg-green-50 border border-green-300 rounded-xl p-3 text-xs text-green-800 font-bold">
            ✅ ¡Al menos un proveedor funciona! Guarda para activarlo en toda la app.
          </div>
        ) : (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {Object.entries(PROVIDER_INFO).map(([k, info]) => (
              <button key={k} type="button"
                onClick={() => setCfg((p) => ({ ...p, activeProvider: k }))}
            ...
            <div key={k} className={`rounded-xl border-2 overflow-hidden ${cfg.activeProvider === k ? c.border : 'border-gray-200'}`}>
              <div className={`flex justify-between items-center p-2.5 ${cfg.activeProvider === k ? c.bg : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${info.badgeClass}`}>{info.badge}</span>
                  {hasKey && <span className="text-[9px] text-green-600 font-black">🟢 Key OK</span>}
                </div>
              ...
              {isGuideOpen && (
                <div className={`p-3 border-t ${c.bg}`}>
                  <ol className="space-y-1">
                    {info.steps.map((step, i) => <li key={i} className="text-[10px] text-gray-700">{step}</li>)}
                  </ol>
                </div>
              )}
              <div className="p-2.5 bg-white">
                <div className="flex gap-1.5">
                  ...
                  <button type="button" onClick={() => setShowKey((p) => ({ ...p, [k]: !p[k] }))}
                    className="text-gray-400 hover:text-gray-600 text-[11px] px-1.5">
                    {showKey[k] ? '🙈' : '👁'}
                  </button>
