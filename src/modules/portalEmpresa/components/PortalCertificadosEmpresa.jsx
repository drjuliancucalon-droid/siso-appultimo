// src/modules/portalEmpresa/components/PortalCertificadosEmpresa.jsx
...
// Migrado desde src/pages/PortalCertificadosEmpresa.jsx — imports ajustados al módulo

import React, { useState, useMemo } from 'react';
import { useParams, useNavigate, useSearchParams } from 'react-router-dom';
import {
  Building2, Receipt, Shield, Download, Printer,
  ChevronDown, ChevronUp, Loader2, AlertCircle,
  FileSpreadsheet, FileCheck, ArrowLeft,
  Search, Filter, Package, Mail
} from 'lucide-react';
...
export default function PortalCertificadosEmpresa() {
  ...
  const company = useMemo(() => {
    if (urlCompanyId) return companies.find(c => c.id === urlCompanyId);
    if (nitFromQuery) {
  ...
  // useCompanyDocuments devuelve 'custodia' — lo aliasamos a 'cartas'
  const {
    certificates = [],
    reports = [],
    bills = [],
  ...
  const filteredCertificates = useMemo(() => {
    ...
    if (selectedPeriod.mes || selectedPeriod.anio) {
      r = r.filter(c => {
        const [anio, mes] = (c.fechaExamen || c.cerradaAt || '').split('-');
    ...
    if (searchTerm) {
      ...
      r = r.filter(c => (c.nombres || '').toLowerCase().includes(t) || (c.docNumero || '').includes(t));
    }
    return r;
  }, [certificates, selectedPeriod, searchTerm]);

  const filteredReports = useMemo(() => {
    if (!selectedPeriod.mes && !selectedPeriod.anio) return reports;
    return reports.filter(r =>
      (r.periodoInicio || '').includes(`${selectedPeriod.anio}-${selectedPeriod.mes}`) ||
      (r.periodoFin || '').includes(`${selectedPeriod.anio}-${selectedPeriod.mes}`)
    );
  }, [reports, selectedPeriod]);

  const filteredBills = useMemo(() => {
    if (!selectedPeriod.mes && !selectedPeriod.anio) return bills;
  ...
  const filteredCartas = useMemo(() => {
    if (!selectedPeriod.mes && !selectedPeriod.anio) return cartas;
    return cartas.filter(c => c.mes === selectedPeriod.mes || c.anio === selectedPeriod.anio);
  }, [cartas, selectedPeriod]);

  const handleDownloadCertificate = (cert) => {
    const doc = doctorData || { nombre: currentUser?.nombre || 'Médico' };
    const sig = signatureData?.signature || null;
    downloadCertificateAsHTML(cert, doc, sig, `certificado_${cert.docNumero}_${cert.nombres?.replace(/\s+/g, '_')}.html`);
  };
  ...
  const handleDownloadAllCertificates = async () => {
    ...
    try {
      const doc = doctorData || { nombre: currentUser?.nombre || 'Médico' };
      await downloadCertificatesAsZip(filteredCertificates, doc, signatureData?.signature || null, company?.nombre || 'Empresa');
    } catch (err) {
      alert('Error al generar ZIP. Intente descargar individualmente.');
    } finally {
  ...
  const handlePrintCertificate = (cert) => {
    const doc = doctorData || { nombre: currentUser?.nombre || 'Médico' };
    const html = _generarCertificadoHTMLNormalizado(cert, doc, signatureData?.signature || null);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-xl border border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-800">{filteredCertificates.length} certificados</span>
        </div>
        <button onClick={handleDownloadAllCertificates} disabled={downloading || !filteredCertificates.length}
      ...
      {filteredCertificates.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filteredCertificates.map((cert) => (
            <div key={cert.id} className="bg-white p-4 rounded-xl border border-gray-200 hover:border-emerald-300 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-gray-900">{cert.nombres}</h3>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                      cert.conceptoAptitud?.toLowerCase().includes('apto') ? 'bg-green-100 text-green-700'
                      : cert.conceptoAptitud?.toLowerCase().includes('no apto') ? 'bg-red-100 text-red-700'
                  ...
                  <div className="text-sm text-gray-500 space-y-1">
                    ...
                    <p><span className="font-medium">Código verificación:</span>
                      <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded ml-1">{cert.codigoVerificacion}</span>
                    </p>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={() => handlePrintCertificate(cert)}
  ...
  const renderInformesTab = () => (
    <div className="space-y-4">
      <div className="flex items-center gap-2 bg-white p-4 rounded-xl border border-gray-200">
        ...
        <span className="font-bold text-gray-800">{filteredReports.length} informes</span>
      </div>
      {filteredReports.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filteredReports.map((report) => (
            <div key={report.id} className="bg-white p-4 rounded-xl border border-gray-200 hover:border-blue-300 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900 mb-1">Informe {report.tipoInforme || 'Epidemiológico'}</h3>
                  <div className="text-sm text-gray-500 space-y-1">
                    <p><span className="font-medium">Período:</span> {formatDate(report.periodoInicio)} — {formatDate(report.periodoFin)}</p>
                    <p><span className="font-medium">Trabajadores:</span> {report.totalTrabajadores || 'N/A'}</p>
                    <p><span className="font-medium">Generado:</span> {formatDate(report.fechaGeneracion)} por {report.generadoPorNombre || report.generadoPor}</p>
                  </div>
                ...
                <button onClick={() => downloadReportAsHTML(report, `informe_${report.periodoInicio}.html`)}
  ...
  const renderCuentasTab = () => (
    <div className="space-y-4">
      <div className="flex items-center gap-2 bg-white p-4 rounded-xl border border-gray-200">
        ...
        <span className="font-bold text-gray-800">{filteredBills.length} cuentas de cobro</span>
      </div>
      {filteredBills.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filteredBills.map((bill) => (
            <div key={bill.id} className="bg-white p-4 rounded-xl border border-gray-200 hover:border-green-300 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-gray-900">Cuenta #{bill.numero}</h3>
                ...
                <button onClick={() => downloadBillAsHTML(bill, `cuenta_${bill.numero}.html`)}
  ...
  const renderCustodiaTab = () => (
    <div className="space-y-4">
      <div className="flex items-center gap-2 bg-white p-4 rounded-xl border border-gray-200">
        ...
        <span className="font-bold text-gray-800">{filteredCartas.length} cartas de custodia</span>
      </div>
      {filteredCartas.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filteredCartas.map((carta) => (
            <div key={carta.id} className="bg-white p-4 rounded-xl border border-gray-200 hover:border-purple-300 transition-colors">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900 mb-1">Carta de Custodia — {formatPeriod(carta.mes, carta.anio)}</h3>
                  <div className="text-sm text-gray-500 space-y-1">
                    <p><span className="font-medium">Fecha:</span> {formatDate(carta.fechaCarta)}</p>
                    ...
                    {carta.enviadaPorEmail && (
                      <p className="text-purple-600 text-xs">
                        ...
                        Enviada a: {carta.emailDestino}
                      </p>
                    )}
                  </div>
                </div>
                <button onClick={() => downloadCustodiaAsHTML(carta)}
  ...
  // ── Estados de carga / error / no empresa ────────────────────────
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6 bg-white rounded-xl border border-red-200">
          ...
          <button onClick={refresh} className="mt-4 px-4 py-2 bg-red-50 text-red-700 rounded-lg text-sm font-bold hover:bg-red-100">
            Reintentar
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-gray-500 text-sm mb-4">No se encontró empresa con el ID o NIT indicado.</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                <p className="text-2xl font-black text-emerald-600">
                  {certificates.length + reports.length + bills.length + (cartas?.length || 0)}
                </p>
          ...
          <div className="flex gap-1 overflow-x-auto pb-1">
            {TABS.map((tab) => {
              ...
              const count = { certificados: certificates.length, informes: reports.length, cuentas: bills.length, custodia: cartas?.length || 0 }[tab.id];
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-t-lg text-sm font-bold whitespace-nowrap transition-colors ${
                    isActive ? `bg-${tab.color}-50 text-${tab.color}-700 border-b-2 border-${tab.color}-500`
                    : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                  <Icon className="w-4 h-4" />
                  {tab.label}
                  <span className={`ml-1 px-1.5 py-0.5 rounded-full text-xs ${isActive ? `bg-${tab.color}-100 text-${tab.color}-700` : 'bg-gray-100 text-gray-600'}`}>
                    {count}
                  </span>
                </button>
              );
          ...
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select value={selectedPeriod.mes} onChange={e => setSelectedPeriod(p => ({ ...p, mes: e.target.value }))}
              ...
              <div>
                ...
                <select value={selectedPeriod.anio} onChange={e => setSelectedPeriod(p => ({ ...p, anio: e.target.value }))}
                  className="w-full p-2 border border-gray-200 rounded-lg text-sm">
                  <option value="">Todos</option>
            ...
            <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
              {selectedPeriod.mes && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-xs">
                  Mes: {selectedPeriod.mes} <button onClick={() => setSelectedPeriod(p => ({ ...p, mes: '' }))}>×</button>
                </span>
              )}
              {selectedPeriod.anio && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-xs">
                  Año: {selectedPeriod.anio} <button onClick={() => setSelectedPeriod(p => ({ ...p, anio: '' }))}>×</button>
                </span>
              )}
              {searchTerm && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-xs">
              ...
              <button onClick={() => { setSelectedPeriod({ mes: '', anio: '' }); setSearchTerm(''); }}
