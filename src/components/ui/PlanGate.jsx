  ...
  if (inline)
    return (
      <span
        ...
        title={isExpired ? "Plan vencido � comun�cate con tu administrador" : `Disponible en plan ${req.label}`}
      >
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-6 text-center space-y-4 shadow-sm">
        ...
        <div>
          ...
          <p className="text-amber-700 text-sm mt-1">
            Tu suscripci�n <strong>{plan.label}</strong> expir� el{" "}
            <strong>{new Date(currentUser.licenseExpiry).toLocaleDateString("es-CO", { day:"numeric", month:"long", year:"numeric" })}</strong>.
            <br/>Tu informaci�n est� segura. Solo necesitas renovar para continuar.
          </p>
        </div>
        <div className="bg-white border border-amber-200 rounded-xl p-4 text-left space-y-2 max-w-sm mx-auto">
          <p className="text-xs font-black text-amber-800 uppercase tracking-wide mb-2">?? Renueva tu plan f�cilmente</p>
          <div className="flex items-center gap-2 text-sm text-gray-700">
            <span className="text-base">?????</span>
            <span><strong>Dr. Juli�n Cucal�n</strong> � OcupaSalud</span>
          </div>
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-blue-50 border-2 border-dashed border-blue-200 rounded-xl p-5 text-center space-y-3">
      ...
      <div>
        ...
        <p className="text-gray-500 text-xs mt-1">{req.priceLabel} � Desbloquea esta y muchas funciones m�s</p>
      </div>
