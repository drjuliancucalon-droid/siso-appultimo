      ...
      <div className="flex items-center justify-end gap-2 mt-2">
          {(() => {
            ...
            const turnoMedicoNombre = storedTurno
              ...
              : null;
            return storedTurno ? (
              <div onClick={() => currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? setShowTurnoModal(true) : null}
                className={`flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-1.5 ${currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? 'cursor-pointer hover:bg-green-100' : ''} transition`}
                title={currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? 'Click para cambiar médico de turno' : 'Médico de turno actual'}>
              ...
              <div onClick={() => currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? setShowTurnoModal(true) : null}
                className={`flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-1.5 ${currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? 'cursor-pointer hover:bg-amber-100' : ''} transition`}
                title={currentUser?.role && ['super_admin','administrador'].includes(currentUser.role) ? 'Click para asignar médico de turno' : 'Sin médico de turno asignado'}>
