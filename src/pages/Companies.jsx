...
import { _secretariaPuede, _secretariaMedicoAsignado, ORG_DEFAULT_ID } from "../shared/data/planConfig.js";
...
const CompaniesPage = (props) => {
  const {
    currentUser, usersList, companies, setCompanies,
    patientsList, editingCompany, setEditingCompany,
  ...
  // Calcular alertas de convenios pr├│ximos a vencer
  const hoy = new Date();
  ...
  // Secretaria: filtra empresas por m├⌐dicos asignados
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {/* Alerta convenios pr├│ximos a vencer */}
          ...
          <div className="bg-amber-50 border border-amber-300 rounded-xl p-3 mb-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black text-amber-800">
                {conveniosAlerta.length} convenio(s) pr├│ximo(s) a vencer:
              </p>
        ...
        <div className="flex gap-1 mb-4 bg-white rounded-xl p-1 shadow-sm border border-gray-100">
          {[
            { k: "lista", l: "≡ƒÅó Empresas" },
            { k: "nueva", l: "Γ₧ò Nueva Empresa" },
            { k: "convenios", l: "≡ƒñ¥ Convenios" },
          ].map((t) => (
            <button
              key={t.k}
          ...
          <div className="space-y-3">
            {companies.length === 0 && (
              <div className="bg-white rounded-2xl p-8 text-center text-gray-400 text-sm">
                Sin empresas registradas. Use Γ₧ò Nueva Empresa.
              </div>
            )}
            {companies.map((c, i) => {
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      {/* FASE 2: multi-m├⌐dico badges */}
                      <div className="flex flex-wrap gap-1 mt-0.5">
                        {medResp && (
                          <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">
                            ≡ƒæ¿ΓÇìΓÜò∩╕Å {medResp.name} Γ¡É
                          </span>
                        )}
                        {(c.medicoIds || [])
                          ...
                          onClick={() => {
                            if (navigator.clipboard) {
                              navigator.clipboard
                                ...
                                .then(() =>
                                  showAlert(
                                    "Γ£à C├│digo " + c.portalCode + " copiado."
                                  )
                                );
                            } else showAlert("C├│digo: " + c.portalCode);
                          }}
        ...
        {companiesTab === "nueva" && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="flex flex-wrap -mx-1.5">
                ...
                <div className="px-1.5 mb-3 w-1/4">
                  ...
                  <select
                    ...
                    className="w-full p-1.5 border rounded-lg text-xs"
                  >
                    {["contado", "30 d├¡as", "60 d├¡as", "90 d├¡as"].map((o) => (
              ...
              <div className="flex gap-4 mt-1">
                ...
                <label className="flex items-center gap-1.5 text-xs font-bold cursor-pointer">
                  <input
                    ...
                    className="accent-purple-600"
                  />{" "}
                  Facturaci├│n agrupada
                </label>
              </div>
            </div>
            {/* ΓöÇΓöÇ MULTI-M├ëDICO (FASE 2) ΓöÇΓöÇ */}
            <div className="border-t border-gray-100 pt-4 mt-2">
              <p className="text-xs font-black text-gray-700 uppercase mb-2">
                ≡ƒæ¿ΓÇìΓÜò∩╕Å M├⌐dicos asignados a esta empresa
              </p>
              <p className="text-[10px] text-gray-500 mb-2">
                El m├⌐dico responsable es el principal; los adicionales tambi├⌐n
              ...
              <div className="flex flex-wrap gap-2">
                {medicos.map((m) => (
                  ...
                  >
                    ...
                    >
                      {m.name || m.user}
                      {m.user === newComp.medicoResponsableId && " Γ¡É"}
                    </span>
                  </label>
                ))}
                {medicos.length === 0 && (
            ...
            {/* ΓöÇΓöÇ SEDES (FASE 2) ΓöÇΓöÇ */}
            <div className="border-t border-gray-100 pt-4 mt-2">
              <p className="text-xs font-black text-gray-700 uppercase mb-2">
                ≡ƒÅó Sedes de la empresa
              </p>
            ...
            {/* ΓöÇΓöÇ ADMIN DEL PORTAL (FASE 2) ΓöÇΓöÇ */}
            {newComp.portalActivo && (
              <div className="border-t border-purple-100 pt-4 mt-2 bg-purple-50 rounded-xl p-3">
                <p className="text-xs font-black text-purple-700 uppercase mb-1">
                  ≡ƒöÉ Acceso Admin del Portal
                </p>
                <p className="text-[10px] text-purple-600 mb-2">
                  El admin de la empresa usar├í estas credenciales para
                  ingresar al portal y gestionar sus m├⌐dicos/secretarias.
                </p>
            ...
            <button
              onClick={async () => {
                if (!newComp.nombre) {
                  showAlert("Ingrese la raz├│n social.");
                  return;
                }
                // Auto-generar c├│digo de portal si est├í activo y no tiene c├│digo a├║n
                let finalComp = {
                  ...newComp,
                ...
                // FASE 2: hashear contrase├▒a admin portal
                ...
                if (finalComp.portalActivo) {
                  ...
                  showAlert("Γ£à Empresa registrada.");
                  setCompaniesTab("lista");
                }
              }}
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="bg-purple-700 p-4 rounded-t-2xl flex justify-between items-center">
                <p className="text-white font-black">
                  Γ£Å∩╕Å Editar: {editingCompany.nombre}
                </p>
                <button
              ...
              <div className="p-4">
                  ...
                  <div className="flex flex-wrap -mx-1.5">
                    ...
                    <div className="px-1.5 mb-2 w-1/3">
                      ...
                      <select
                        ...
                        className="w-full p-1.5 border rounded-lg text-xs"
                      >
                        {["contado", "30 d├¡as", "60 d├¡as", "90 d├¡as"].map(
                          (o) => (
                            <option key={o} value={o}>
                              {o}
                            </option>
                    ...
                    {editingCompany.portalCode ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            showAlert(
                              "≡ƒöä C├│digo regenerado: " +
                                newCode +
                                "\n\nΓÜá∩╕Å Guarda los cambios y env├¡a el nuevo c├│digo a la empresa."
                            );
                          }}
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-[10px] text-amber-700 flex-1">
                          Sin c├│digo generado - se crear├í al guardar
                        </p>
                      </div>
                    )}
                  </div>
                )}
                {/* FASE 2: Multi-m├⌐dico en editar empresa */}
                <div className="border-t border-gray-100 pt-3 mt-2">
                  <p className="text-xs font-black text-gray-700 uppercase mb-2">
                    ≡ƒæ¿ΓÇìΓÜò∩╕Å M├⌐dicos asignados
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {medicos.map((m) => (
                      <label
                ...
                {editingCompany.portalActivo && (
                  <div className="border-t border-purple-100 pt-3 mt-2 bg-purple-50 rounded-xl p-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="flex-1">
                        <label className="block text-[10px] font-black text-purple-700 mb-1">
                          Nueva contrase├▒a (vac├¡o = sin cambio)
                        </label>
                        <input
                          type="password"
                    ...
                    {editingCompany.portalAdminPassHash &&
                      !editingCompany.portalAdminPassPlain && (
                        <p className="text-[10px] text-purple-500 mt-1">
                          Γ£à Admin configurado. Dejar vac├¡a la contrase├▒a para
                          no cambiarla.
                        </p>
                      )}
                  </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    // FASE 2: hashear contrase├▒a admin portal si se proporcion├│
                    ...
                    // Auto-generar c├│digo si portal activo y no tiene c├│digo
                    ...
                    if (saved.portalActivo) {
                      ...
                      showAlert("Γ£à Empresa actualizada.");
                    }
                  }}
                  className="w-full mt-4 bg-purple-700 hover:bg-purple-800 text-white py-2.5 rounded-xl text-sm font-black"
                >
      ...
      {/* ΓòÉΓòÉΓòÉ MODAL PORTAL ACTIVADO ΓòÉΓòÉΓòÉ */}
      {portalActivadoInfo &&
        (() => {
          const baseUrl = window.location.href.split("#")[0];
          const portalUrl = `${baseUrl}#portalempresa?code=${portalActivadoInfo.portalCode}`;
          const msgWhatsapp = [
            `≡ƒÅó *Portal SISO OcupaSalud - ${portalActivadoInfo.nombre}*`,
            ``,
            `Estimado cliente, su portal de seguimiento m├⌐dico ya est├í disponible.`,
            ``,
            `≡ƒôï *┬┐Qu├⌐ puede ver en su portal?*`,
            `Γ£à Listado de trabajadores evaluados`,
            `Γ£à Conceptos de aptitud laboral`,
            `Γ£à Estado de cuentas y pagos`,
            `Γ£à Trabajadores con restricciones activas`,
            `≡ƒöÆ Los diagn├│sticos m├⌐dicos son CONFIDENCIALES y no se muestran`,
            ``,
            ...
            `2. Inicio ΓåÆ bot├│n ≡ƒÅó Portal Empresa`,
            `3. Escribir c├│digo: *${portalActivadoInfo.portalCode}*`,
            `   (o su NIT: ${portalActivadoInfo.nit})`,
            ``,
            `Saludos,`,
            `${currentUser?.name || "Su m├⌐dico ocupacional"}`,
          ].join("\n");
          return (
            <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 overflow-y-auto">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl my-4">
                ...
                <div className="p-5 space-y-4 overflow-y-auto max-h-[75vh]">
                  {/* ΓöÇΓöÇΓöÇ C├ôMO FUNCIONA ΓöÇΓöÇΓöÇ */}
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                    <p className="text-xs font-black text-blue-800 mb-1">
                      ≡ƒôî ┬┐C├│mo funciona el portal por empresa?
                    </p>
                    <p className="text-[11px] text-blue-700 leading-relaxed">
                      SISO usa <strong>un portal inteligente</strong> que cada
                      empresa accede con su c├│digo exclusivo. Al ingresar el
                      c├│digo, el portal filtra y muestra{" "}
                      <strong>├║nicamente los datos de esa empresa</strong>.
                      Ninguna empresa puede ver datos de otra.
                    </p>
                  </div>

                  {/* ΓöÇΓöÇΓöÇ C├ôDIGO ΓöÇΓöÇΓöÇ */}
                  <div className="bg-indigo-50 border-2 border-indigo-400 rounded-xl p-4">
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-wider mb-2 text-center">
                      ≡ƒöæ C├│digo de acceso exclusivo
                    </p>
                        ...
                        onClick={() => {
                          if (navigator.clipboard) {
                            navigator.clipboard
                              ...
                              .then(() => showAlert("Γ£à C├│digo copiado"));
                          } else
                            showAlert(
                              "C├│digo: " + portalActivadoInfo.portalCode
                            );
                        }}
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                    ...
                    <p className="text-[10px] text-emerald-600 mt-1">
                      Al abrir este enlace el c├│digo se pre-carga - la empresa
                      solo presiona "Acceder"
                    </p>
                  </div>

                  {/* ΓöÇΓöÇΓöÇ QU├ë VE ΓöÇΓöÇΓöÇ */}
                  <div>
                    <p className="text-[10px] font-black text-gray-600 uppercase mb-2">
                      Lo que ver├í <strong>{portalActivadoInfo.nombre}</strong>
                      :
                    </p>
                    ...
                    <p className="text-[10px] text-red-600 font-bold text-center mt-2">
                      ≡ƒöÆ Diagn├│sticos cl├¡nicos = CONFIDENCIALES - nunca
                      visibles (Art. 16 Res. 1843/2025)
                    </p>
                  </div>

                  {/* ΓöÇΓöÇΓöÇ PR├ôXIMOS PASOS ΓöÇΓöÇΓöÇ */}
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                    ...
                    <div className="space-y-2">
                      {[
                        {
                          n: "1",
                          txt: `Copie el mensaje de abajo y env├¡elo al gerente o contacto de ${portalActivadoInfo.nombre} por WhatsApp o correo`,
                        },
                        {
                          n: "2",
                          txt: "La empresa abre el enlace directo (o ingresa el c├│digo manualmente en Portal Empresa)",
                        },
                        {
                          n: "3",
                          txt: "Ver├ín en tiempo real sus trabajadores evaluados, aptitudes y estado de cuentas",
                        },
                        {
                          n: "4",
                          txt: "Cada nueva HC cerrada de esta empresa aparecer├í autom├íticamente en su portal",
                        },
                      ].map((s) => (
                    ...
                    onClick={() => {
                      if (navigator.clipboard) {
                        navigator.clipboard
                          ...
                          .then(() =>
                            showAlert(
                              "Γ£à Mensaje copiado.\n\nP├⌐guelo en WhatsApp, correo o Teams."
                            )
                          );
                      } else {
