...
export default function PatientsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      let list = [];
      try { const { value: v } = await d1Get(`siso_patients_${userId}`, { userId }); if (Array.isArray(v) && v.length > 0) { list = v; if (!cancelled) setSource('d1'); } } catch {}
      if (list.length === 0 && !cancelled) {
        try { const { value: v } = await d1Get(`siso_patients_${userId}`, { userId }); if (Array.isArray(v) && v.length > 0) { list = v; if (!cancelled) setSource('d1-fallback'); } } catch {}
      }
      if (list.length === 0 && !cancelled) {
        try {
  ...
  const handleCreatePatient = useCallback(async (e) => {
    ...
    try {
      ...
      await d1WriteArrayMerge(`siso_patients_${userId}`, [paciente], 'docNumero', { userId });
