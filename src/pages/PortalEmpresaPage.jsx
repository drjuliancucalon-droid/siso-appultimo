                          ...
                          <div className="p-4 border-t border-gray-100 space-y-3">
                            {certsDelPeriodo.length > 0 && (
                              <div className="flex items-center justify-between p-2 bg-blue-50 rounded-lg">
                                ...
                                <button onClick={() => {
                                  const html = certsDelPeriodo.map((p, idx) => {
                                    const d = p; const certHtml = `<div style="text-align:center;margin-bottom:16px"><h2 style="color:#065f46">CERTIFICADO DE APTITUD OCUPACIONAL</h2><p style="font-size:9pt;color:#666">Res. 1843/2025 — ${formatDate(d.fechaExamen)}</p></div><div style="font-size:10pt"><p><strong>${d.nombres||''}</strong>, ${d.docTipo||'CC'} ${d.docNumero||''}, cargo ${d.cargo||'—'} en ${d.empresaNombre||''}, evaluado el ${formatDate(d.fechaExamen)}, tipo: ${d.tipoExamen||'—'}.</p><p style="font-size:14pt;font-weight:900;color:#065f46;margin-top:16px;text-align:center">Concepto: ${d.conceptoAptitud||'—'}</p></div>`;
                                    return `<div style="${idx > 0 ? 'page-break-before:always;' : ''}padding:12px">${certHtml}</div>`;
                                  }).join('');
                                  const w = window.open('', '_blank', 'width=900,height=700');
                                  if (w) { w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Certificados ${per}</title><style>@page{size:letter;margin:14mm}body{font-family:Arial;font-size:10pt;color:#111}@media print{body{padding:0!important}}</style></head><body><div style="position:fixed;top:10px;right:10px;z-index:9999"><button onclick="window.print()" style="background:#065f46;color:#fff;border:none;padding:10px 24px;border-radius:10px;font-weight:900;cursor:pointer;font-size:12px">🖨️ Imprimir (${certsDelPeriodo.length})</button></div>${html}</body></html>`); w.document.close(); }
                            ...
                            {data.cuenta && (
                              <div className="flex items-center justify-between p-2 bg-amber-50 rounded-lg">
                                ...
                                <button onClick={() => { const c = data.cuenta; const h='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cuenta de Cobro</title><style>@page{size:letter;margin:2cm}body{font-family:Arial;font-size:11pt}h2{color:#059669;border-bottom:2px solid #059669}table{width:100%;border-collapse:collapse;margin:12px 0}td,th{border:1px solid #ddd;padding:8px}th{background:#f0fdf4}.total{font-size:14pt;font-weight:900;color:#059669;text-align:right}</style></head><body><h2>Cuenta de Cobro</h2><p><b>Empresa:</b> '+(authenticated?.nombre||'—')+'</p><p><b>NIT:</b> '+(authenticated?.nit||'—')+'</p><p><b>Fecha:</b> '+(c.fecha||'—')+'</p><p><b>Concepto:</b> '+(c.concepto||'—')+'</p><table><tr><th>Descripción</th><th>Cantidad</th><th>Valor</th><th>Total</th></tr><tr><td>'+(c.concepto||'Servicios')+'</td><td>1</td><td>$'+Number(c.monto||0).toLocaleString('es-CO')+'</td><td>$'+Number(c.monto||0).toLocaleString('es-CO')+'</td></tr></table><p class="total">TOTAL: $'+Number(c.monto||0).toLocaleString('es-CO')+'</p><script>setTimeout(()=>window.print(),400)</'+'script></body></html>'; const w=window.open('','_blank','width=900,height=700');if(w){w.document.write(h);w.document.close()} }} className="px-3 py-1.5 bg-amber-600 text-white rounded-lg text-[10px] font-black hover:bg-amber-700 flex items-center gap-1"><Download size={12}/> Descargar</button>
                              </div>
                            )}
                            {data.custodia && (
                              <div className="flex items-center justify-between p-2 bg-emerald-50 rounded-lg">
                                ...
                                <button onClick={() => { const h='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Carta Custodia</title><style>@page{size:letter;margin:2cm}body{font-family:Arial;font-size:10pt}h2{color:#059669;border-bottom:2px solid #059669}</style></head><body><h2>Carta de Custodia</h2><p><b>Empresa:</b> '+(authenticated?.nombre||'—')+'</p><p>Se certifica custodia conforme Res. 1995/1999 por 20 años.</p><div style="margin-top:40px;border-top:1px solid #333;padding-top:8px;width:50%"><p style="font-weight:700">'+(authenticated?.nombre||'Médico')+'</p></div><script>setTimeout(()=>window.print(),400)</'+'script></body></html>'; const w=window.open('','_blank','width=900,height=700');if(w){w.document.write(h);w.document.close()} }} className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-[10px] font-black hover:bg-emerald-700 flex items-center gap-1"><Download size={12}/> Ver</button>
                              </div>
                            )}
                            {data.informe && (
