...
const CajaPage = (props) => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 font-sans flex items-center justify-center">
        ...
        <div className="bg-white rounded-2xl p-8 shadow text-center max-w-sm">
          <p className="text-4xl mb-3">≡ƒöÆ</p>
          <p className="font-black text-gray-800 mb-1">Acceso restringido</p>
          <p className="text-xs text-gray-500">
            El m├│dulo financiero no est├í habilitado para su perfil. Solicite
  ...
  // ΓöÇΓöÇ IPS: scope caja movements to empresa ΓöÇΓöÇ
  ...
  const handleAddMov = () => {
    ...
    if (
      ...
      showAlert("Ingrese un monto v├ílido mayor a 0.");
      return;
    }
    const nuevo = {
      ...cajaForm,
  ...
  const deleteMovimiento = (id) => {
    if (currentUser?.role === "secretaria") {
      showAlert(
        "Γ¢ö Solo el administrador puede eliminar movimientos de caja."
      );
      return;
    }
  ...
  // ΓöÇΓöÇ IPS: scope billing to empresa ΓöÇΓöÇ
  ...
  const editarCuenta = (bill) => {
    showPrompt("C├│digo de seguridad para editar:", (secCode) => {
      if (secCode !== "9207") {
        showAlert("Γ¢ö C├│digo incorrecto.");
        return;
      }
      showPrompt(
        "Nuevo monto (actual: $" +
  ...
  const eliminarCuenta = (id) => {
    showPrompt("C├│digo de seguridad para eliminar:", (code) => {
      if (code !== "9207") {
        showAlert("Γ¢ö C├│digo incorrecto.");
        return;
      }
  ...
  // Agrupaci├│n de cuentas por empresa/convenio
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 mb-4 flex flex-wrap gap-2 items-center">
          ...
          {cajaFiltroPeriodo === "rango" && (
            <div className="flex gap-2 items-center ml-2">
              ...
              />
              <span className="text-gray-400 text-xs">ΓÇô</span>
              <input
                type="date"
        ...
        <div className="flex flex-wrap gap-1 mb-4 bg-white rounded-xl p-1 shadow-sm border border-gray-100">
          {[
            { k: "hoy", l: "≡ƒÆ╡ Caja del D├¡a" },
            { k: "historial", l: "≡ƒô£ Historial" },
            { k: "comprobantes", l: "≡ƒôï Comprobantes" },
            {
              k: "cuentas",
              l: `≡ƒÆ│ Cuentas (${cuentasPendientes.length} pend.)`,
            },
            {
              k: "pacientes_vistos",
              l: `≡ƒæü∩╕Å Pacientes Vistos (${pacientesVistosCount})`,
            },
            ...(_isAdmin(currentUser?.role)
              ? [{ k: "por_medico", l: "≡ƒæ¿ΓÇìΓÜò∩╕Å Por M├⌐dico" }]
              : []),
          ].map((t) => (
            <button
              key={t.k}
        ...
        {cajaTab === "hoy" && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <div>
                  ...
                  >
                    <option value="ingreso">≡ƒÆÜ Ingreso</option>
                    <option value="egreso">Γ¥ñ∩╕Å Egreso</option>
                  </select>
                </div>
            ...
            {/* ΓöÇΓöÇ PACIENTES DE HOY - checklist cobro ΓöÇΓöÇ */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
              <p className="text-xs font-black text-gray-700 uppercase mb-3">
                ≡ƒæÑ Pacientes de hoy ({pacientesHoy.length}) - Checklist de
                cobro
              </p>
        ...
        {cajaTab === "comprobantes" && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <p className="text-xs font-black text-gray-700 uppercase mb-3">
              ≡ƒôï Comprobantes de Contabilidad
            </p>
            <p className="text-xs text-gray-500 mb-4">
              Seleccione cualquier movimiento del historial y pulse ≡ƒû¿∩╕Å para
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                },
                {
                  ...
                  desc: "Recibo simple de transacci├│n",
                  tipo: "recibo",
                  color: "blue",
                },
                {
                ...
                >
                  ...
                  >
                    ≡ƒû¿∩╕Å Generar en blanco
                  </button>
                </div>
              ))}
            </div>
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
              ≡ƒÆí Para generar comprobantes de movimientos ya registrados, vaya
              a <strong>≡ƒô£ Historial</strong> y pulse el ├¡cono ≡ƒû¿∩╕Å
            </div>
          </div>
        )}
        ...
        {cajaTab === "cuentas" && (
          <div className="space-y-4">
            ...
            {/* ΓöÇΓöÇ RESUMEN POR EMPRESA ΓöÇΓöÇ */}
                                        ...
                                        (mto) => {
                                          showPrompt(
                                            ...
                                            (forma) => {
                                              ...
                                              showAlert(
                                                "Γ£à Movimiento marcado como cobrado."
                                              );
                                            }
                                          );
                                        }
        ...
        {/* TAB: INGRESOS POR M├ëDICO - solo admin */}
        {cajaTab === "por_medico" &&
          _isAdmin(currentUser?.role) &&
          (() => {
            ...
            return (
              <div className="space-y-4">
                ...
                <button
                  onClick={() => {
                    const rows = [
                      ["M├⌐dico", "Atenciones", "Cobrado", "Pendiente"],
                    ];
                    medicos.forEach((med) => {
                      const movsMed = movsFiltro.filter(
                        (m) =>
                ...
                {/* ΓöÇΓöÇ LIQUIDACI├ôN M├ëDICO-CL├ìNICA (FASE 4) ΓöÇΓöÇ */}
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="flex items-center gap-4 bg-indigo-50 rounded-xl p-3">
                      <div className="flex-1">
                        ...
                        <div className="flex justify-between text-[10px] text-gray-400 mt-0.5">
                          <span>0% m├⌐dico</span>
                          <span>100% m├⌐dico</span>
                        </div>
                      </div>
                      <div className="text-center min-w-[90px]">
                        <p className="text-[10px] text-gray-500">Cl├¡nica</p>
                        <p className="text-lg font-black text-purple-700">
                          {100 - porcentajeMedico}%
                        </p>
                      </div>
                    </div>
                    {/* Tabla por m├⌐dico con liquidaci├│n */}
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead className="bg-indigo-700 text-white">
                          <tr>
                            {[
                              "M├⌐dico",
                              "Cobrado",
                              `M├⌐dico (${porcentajeMedico}%)`,
                              `Cl├¡nica (${100 - porcentajeMedico}%)`,
                            ].map((h) => (
                              <th
                                key={h}
                    ...
                    <button
                      onClick={() => {
                        ...
                        const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Liquidaci├│n SISO</title><style>body{font-family:Arial,sans-serif;padding:32px;color:#1f2937}.doc-header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #3730a3;padding-bottom:12px;margin-bottom:16px;}h1{font-size:18px;color:#3730a3;margin:0 0 2px 0;}h2{font-size:13px;color:#6b7280;font-weight:normal;margin-top:2px}table{width:100%;border-collapse:collapse;margin-top:20px}th{background:#3730a3;color:white;padding:8px 10px;text-align:left;font-size:12px}td{font-size:12px}.total-row td{background:#3730a3;color:white;font-weight:bold;padding:8px 10px;text-align:right}.total-label{text-align:left!important}footer{margin-top:32px;font-size:10px;color:#9ca3af}</style></head><body><div class="doc-header">${_liqLeftHtml}<div><h1>Comprobante de Liquidaci├│n M├⌐dico-Cl├¡nica</h1><h2>Periodo: ${cajaMedicoPeriodo} &nbsp;|&nbsp; Distribuci├│n: M├⌐dico ${porcentajeMedico}% / Cl├¡nica ${
                          100 - porcentajeMedico
                        }%</h2></div></div><table><thead><tr><th>M├⌐dico</th><th style="text-align:right">Cobrado</th><th style="text-align:right">Honorarios m├⌐dico</th><th style="text-align:right">Ingreso cl├¡nica</th></tr></thead><tbody>${filas}<tr class="total-row"><td class="total-label">TOTAL</td><td style="text-align:right">$${totalGlobal.toLocaleString(
                          "es-CO"
                    ...
                    >
                      ≡ƒû¿∩╕Å Imprimir Comprobante de Liquidaci├│n
                    </button>
                  </div>
                </div>
              </div>
