...
import { MEDICAMENTOS_CO_BASE, getCustomMeds, addCustomMed } from '../data/medicamentos.js';

export const MedicamentoAutocomplete = ({ value, onChange, placeholder, onSelectMed }) => {
  ...
  useEffect(() => {
    ...
    allMeds.forEach((med) => {
      if (med.g.toLowerCase().includes(q))
        res.push({ label: med.g, sub: `${med.cat} · ${med.dosis}`, full: med.g, dosis: med.dosis, presentaciones: med.p, isGeneric: true });
      med.p.forEach((p) => {
        if (p.toLowerCase().includes(q))
          res.push({ label: p, sub: `${med.g} · ${med.cat}`, full: p, dosis: med.dosis, presentaciones: med.p, isGeneric: false });
      });
    });
  ...
  const handleAddCustom = () => {
    ...
    const exists = allMeds.some((m) => m.g.toLowerCase() === query.toLowerCase() || m.p.some((p) => p.toLowerCase() === query.toLowerCase()));
    if (!exists) {
      const newEntry = { g: query.trim(), p: [query.trim()], cat: "Personalizado", dosis: "Según prescripción" };
      addCustomMed(newEntry);
      setCustomMeds((prev) => [...prev, newEntry]);
    }
    onChange(query.trim());
    if (onSelectMed) onSelectMed({ label: query.trim(), dosis: "", presentaciones: [query.trim()] });
    setSuggestions([]);
    setShow(false);
  };
  ...
  return (
    <div className="relative" ref={ref}>
      <div className="flex gap-1">
        ...
        />
        <button type="button" onClick={handleAddCustom} title="Agregar como medicamento personalizado"
      ...
      {show && suggestions.length > 0 && (
        <div className="absolute z-50 bg-white border border-emerald-200 rounded-xl shadow-xl mt-1 w-full max-h-52 overflow-y-auto">
          {suggestions.map((s, i) => (
            <button key={i} type="button" onClick={() => { setQuery(s.label); onChange(s.label); if (onSelectMed) onSelectMed(s); setSuggestions([]); setShow(false); }}
              ...
              <div className="flex items-center gap-1.5">
                ...
                {s.isGeneric && <span className="text-[9px] bg-emerald-100 text-emerald-700 px-1 rounded">Genérico</span>}
              </div>
